...
class RepositoryTest extends DatabaseTestCase
{
    ...
    public function testOneWildcard()
    {
        $repo = new RepositoryTester();
        $repo->registerBackend(new DatabaseRepositoryBackend($this->dbLink));

        $bob = $repo->oneCustomer(['name' => 'Bob Fanger']);
        $this->assertEquals('1', $bob->id);

        try {
            $bob = $repo->oneCustomer(['id >=' => '0']);
    ...
    public function testCustomerNotFoundException()
    {
        $repo = new RepositoryTester();
        $repo->registerBackend(new DatabaseRepositoryBackend($this->dbLink));
        $this->expectException('Exception', 'Record "id = \'-1\'" doesn\'t exist in "customers"');
        @$repo->getCustomer('-1'); // Invalid/not-existing ID
    }

    public function testDetectIdTruncation()
    {
        $repo = new RepositoryTester();
        $repo->registerBackend(new DatabaseRepositoryBackend($this->dbLink));
        ...
        $this->expectException('Exception', 'The $id parameter doesn\'t match the retrieved data. {1s} != {1}');
        $customer1s = $repo->getCustomer('1s');
    }

    public function testGetRepositoryFunction()
    {
        ...
        // $this->expectException(Warning::class, 'Property "superpowers" doesn\'t exist in a Generated\Customer object'); // Show an notice when setting a non-existing property
    ...
    public function testBelongsTo()
    {
        ...
        // $this->expectException('PHPUnit_Framework_Error_Notice', 'This placeholder belongs to an other object');
        // $clone->name = 'Clone';
    }
    ...
    public function testHasManyIteratorInterface()
    {
        $repo = new RepositoryTester();
        $repo->registerBackend(new DatabaseRepositoryBackend($this->dbLink));
    ...
    public function testHasManyArrayAccessInterface()
    {
        // Test array access
        ...
        // $this->expectException('PHPUnit_Framework_Error_Notice', 'This placeholder is already replaced');
    ...
    public function testGetWildcardPreload()
    {
        $repo = new RepositoryTester();
        $repo->registerBackend(new DatabaseRepositoryBackend($this->dbLink));

        $order = $repo->getOrder(2, ['preload' => true]);
    ...
    public function testSaveWildcard()
    {
        ...
        $c2->orders[] = $repo->createOrder(['product' => 'Scuba gear']);
        unset($c2->orders[1]);
    ...
    public function testReloadWildcard()
    {
        ...
        $repo->reloadCustomer(1, ['discard_changes' => true]);
        ...
        $repo->reloadCustomers(['discard_changes' => true]);
        $this->assertEquals($c1->name, 'Bob Fanger');
        $this->assertEquals($c2->name, 'James Bond');
    }

    public function testAutoCompleteHelper()
    {
